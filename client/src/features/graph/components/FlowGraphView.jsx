...
export default function FlowGraphView() {
  ...
  const emptyMessage = rawData?.message || 'No JS/TS files found in the selected directory.';
  ...
  useEffect(() => {
    ...
    if (!instance || hasFittedViewRef.current || nodes.length === 0) return;
  ...
  const onNodeDoubleClick = useCallback(
    async (_event, node) => {
      ...
      try {
        ...
        if (functionDeclarations.length === 0) {
          setNodes((previousNodes) =>
            previousNodes.map((existingNode) =>
              existingNode.id === node.id
                ? { ...existingNode, style: { ...existingNode.style, boxShadow: '0 0 0 2px #888, 0 0 8px #88888844' } }
                : existingNode,
            ),
          );
          ...
          setTimeout(() => {
            setNodes((previousNodes) =>
              previousNodes.map((existingNode) =>
                existingNode.id === node.id
                  ? { ...existingNode, style: { ...existingNode.style, boxShadow: undefined } }
                  : existingNode,
              ),
            );
          }, 800);
        ...
        console.warn('[FlowGraphView] Failed to load function nodes:', error.message);
      }
    },
      ...
      <div className="flex h-full w-full items-center justify-center px-4">
        <div className="max-w-xl text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Check that the analyzed repository has parseable files. Supported file types include .js, .ts, .py, .go, or .rs files.
          </p>
        </div>
      </div>
    );
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="relative flex-1 min-h-0">
        ...
        >
          ...
          <div className="absolute bottom-14 left-3 z-10 rounded-lg border border-border bg-card/90 p-3 text-[11px] shadow-lg backdrop-blur-sm">
            {heatmapMode ? (
              <>
                <div className="mb-2 font-medium text-muted-foreground">Complexity heatmap</div>
                <div className="mb-1 flex items-center gap-2">
                  <span className="inline-block size-2.5 shrink-0 rounded-sm" style={{ background: '#22c55e' }} />
                  <span className="text-muted-foreground">Low risk</span>
                </div>
                <div className="mb-1 flex items-center gap-2">
                  <span className="inline-block size-2.5 shrink-0 rounded-sm" style={{ background: '#f59e0b' }} />
                  <span className="text-muted-foreground">Medium risk</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="inline-block size-2.5 shrink-0 rounded-sm" style={{ background: '#ef4444' }} />
                  <span className="text-muted-foreground">High risk</span>
                </div>
              </>
            ) : (
              Object.entries(getTypeColors(themeMode)).map(([type, { border }]) => (
                <div key={type} className="mb-1 flex items-center gap-2 last:mb-0">
                  <span className="inline-block size-2.5 shrink-0 rounded-sm" style={{ background: border }} />
